...
export function generateHighlightCSS(
	...
	for (const palette of palettes) {
		...
		const userPadding = palette.padding?.trim() || "";
		const userMargin = palette.margin?.trim() || "";
		const baseClass = `.hltr-m-${palette.id}`;

		// Helper to apply padding/margin: use user override if set, otherwise template default
		const pad = (defaultVal: string): string =>
			`  padding: ${userPadding || defaultVal} !important;`;
		const mar = (defaultVal: string): string =>
			userMargin
				? `  margin: ${userMargin} !important;`
				: `  margin: ${defaultVal} !important;`;
